...
cases <- ggplot(data = covid_data,aes(x=days_since_first_case,y=case_percent,fill=county,color = county)) +
  geom_point() + geom_line() +
  scale_color_manual(values = c(collier_color,cook_color,hennepin_color,philly_color,sumter_color)) +
  geom_vline(xintercept = philly_lockdown_days,color=philly_color,show.legend = T) + 
  geom_text(aes(x=philly_lockdown_days-1, y=.8*max(case_percent),label="Philly lockdown"), 
            colour=philly_color, angle=90) +
  geom_vline(xintercept = cook_lockdown_days,color = cook_color) + 
  geom_text(aes(x=cook_lockdown_days-1, y=.8*max(case_percent),label="Cook lockdown"), 
            colour=cook_color, angle=90) +
  geom_vline(xintercept = hennepin_lockdown_days,color = hennepin_color) +
  geom_text(aes(x=hennepin_lockdown_days-1, y=.8*max(case_percent),label="Hennepin lockdown"), 
            colour=hennepin_color, angle=90) +
  geom_vline(xintercept = collier_lockdown_days,color = collier_color) +
  geom_text(aes(x=collier_lockdown_days-1, y=.8*max(case_percent),label="Florida lockdown"), 
            colour=collier_color, angle=90) +
  geom_vline(xintercept = sumter_lockdown_days,color = sumter_color) +
  geom_text(aes(x=sumter_lockdown_days-1, y=.8*max(case_percent),label="Florida lockdown"), 
            colour=sumter_color, angle=90) +
  labs(y = "Percent of max cases",x="Days since first case",title = "COVID-19 cases")


deaths <- ggplot(data = covid_data,aes(x=days_since_first_case,y=death_percent,fill=county,color = county)) +
  geom_point() + geom_line() +
  scale_color_manual(values = c(collier_color,cook_color,hennepin_color,philly_color,sumter_color)) +
  geom_vline(xintercept = philly_lockdown_days,color=philly_color,show.legend = T) + 
  geom_text(aes(x=philly_lockdown_days-1, y=.8*max(death_percent),label="Philly lockdown"), 
            colour=philly_color, angle=90) +
  geom_vline(xintercept = cook_lockdown_days,color = cook_color) + 
  geom_text(aes(x=cook_lockdown_days-1, y=.8*max(death_percent),label="Cook lockdown"), 
            colour=cook_color, angle=90) +
  geom_vline(xintercept = hennepin_lockdown_days,color = hennepin_color) +
  geom_text(aes(x=hennepin_lockdown_days-1, y=.8*max(death_percent),label="Hennepin lockdown"), 
            colour=hennepin_color, angle=90) +
  geom_vline(xintercept = collier_lockdown_days,color = collier_color) +
  geom_text(aes(x=collier_lockdown_days-1, y=.8*max(death_percent),label="Florida lockdown"), 
            colour=collier_color, angle=90) +
  geom_vline(xintercept = sumter_lockdown_days,color = sumter_color) +
  geom_text(aes(x=sumter_lockdown_days-1, y=.8*max(death_percent),label="Florida lockdown"), 
            colour=sumter_color, angle=90) +
  labs(y = "Percent of max deaths",x="Days since first case",title = "COVID-19 deaths")
